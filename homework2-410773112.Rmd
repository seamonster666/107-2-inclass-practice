...
names(commitCheck) <- str_c("commit",aws)
names(commitCheck)
...
```{r ans25}
dtObject<-ymd_hms(allCommitDates) 
allCommitDatesTW <- ymd_hms(dtObject, tz="Asia/Taipei")
allCommitDatesTW
```
...
```{r ans26}
rty <- hour(allCommitDatesTW)>=4 & hour(allCommitDatesTW)<=5 & minute(allCommitDatesTW)>=30 
rgu <- hour(allCommitDatesTW)>=5 & hour(allCommitDatesTW)<=6
rru <- hour(allCommitDatesTW)>=6 & hour(allCommitDatesTW)<=7 & minute(allCommitDatesTW)<=30
logicWithinTimeFrame <- rty | rgu |rru
...
```{r ans31}
variableName <- names(transcriptData)
class(variableName)
variableName
```
...
```{r ans32}
azx <- transcriptData$學系
logicFromEcon <- azx=="經濟學系"
logicFromEcon
...
```{r ans33}
transcriptData <- transcriptData[c(1,7,8)] 
transcriptData
...
transcriptPartialDataEcon <- list(
  學號=transcriptData$學號[c(logicFromEcon)] ,
  學分數 =transcriptData$學分數[c(logicFromEcon)] ,
  `必選修類別（必∕選∕通）` = transcriptData$`必選修類別（必∕選∕通）`[c(logicFromEcon)]
)
