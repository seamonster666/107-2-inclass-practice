...
假設你叫王小明，學號41078392。
  ...
  2. 本文檔開始的Frontmatter中，studentID（即key）其value目前為"你的學號"，以上改成"王面學號為例則必需改成"41078392"；而studentName（key）其value目前為"你的名字"，以上面名字為例則必需小明"
...
```{r ans13}
ecograde <-c(50,60,92,88)
ecograde2 <- sum(ecograde*fromEcon)
meanEco <- ecograde2/sum(sumEcon=="經濟系")
meanEco
```
...
```{r ans32}
sum((studentID=="046S6f645" & schoolYear=="100")*credit)
```

### 3.3
請問學號為046S6f645的學生在100學年實得了多少學分？答案請存在物件earnedCredit100ChosenStudent。
```{r ans33}
earnedCredit100ChosenStudent <- studentID=="046S6f645" & earnedCredit!=0 & schoolYear==100
sum(earnedCredit100ChosenStudent*credit)
```
...
```{r ans34}
studenta <- studentID=="046S6f645" & schoolYear==100 & credit>0
gpa100ChosenStudent<- sum(studenta*grade*credit)/sum((studentID=="046S6f645" & schoolYear=="100")*credit)
gpa100ChosenStudent
```
