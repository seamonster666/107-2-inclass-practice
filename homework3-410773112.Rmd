...
```{r ans11}
wwwq <- finStatement$`營業利益（損失）`< 0
loss1Companies <- finStatement$公司名稱[wwwq]
loss1Companies
```
...
```{r ans12}
wers <- finStatement$`基本每股盈餘（元）`< 0
loss2Companies <- finStatement$公司名稱[wers]
loss2Companies
```
...
```{r ans13}
wsa <- finStatement$`營業利益（損失）` > 0 & finStatement$`基本每股盈餘（元）` < 0
asaz <- finStatement$`營業利益（損失）` < 0 & finStatement$`基本每股盈餘（元）` > 0
xzx <- wsa | asaz
only1LossCompanies <- finStatement $公司名稱[xzx]
only1LossCompanies
```
...
```{r ans22}
df_oneShow <- data.frame(title=exhibitionList[[1]]$title,time=exhibitionList[[1]]$showInfo[[1]]$time,
                         location=exhibitionList[[1]]$showInfo[[1]]$location,locationName=exhibitionList[[1]]$showInfo[[1]]$locationName,price=exhibitionList[[1]]$showInfo[[1]]$price,endTime=exhibitionList[[1]]$showInfo[[1]]$endTime,sourceWebName=exhibitionList[[1]]$sourceWebName,stringsAsFactors=F)
df_oneShow
```
...
```{r ans23}
dataframe_show_info <- function(exhibitionInput){
 data.frame( title=exhibitionInput$title,
...
```{r ans25}
tyrty <- sum(df_allShows$sourceWebName=="全國藝文活動資訊系統")
totalCount <- as.numeric(tyrty)
totalCount
```
